...
- Transcript mode sessions use one conversation-owned coordinator to connect guarded computer-use, browser, and camera sources to immutable message membership. Source retirement stops future claims while captured turns and accepted frame writes settle. Durable lifecycle and display-boundary writes use compare-and-swap revisions, and startup interrupts every record left active before database readiness while preserving its last confirmed activity and unknown end. History batches session descriptors with messages and uses the existing conversation-messages invalidation path. See [`assistant/docs/mode-sessions.md`](assistant/docs/mode-sessions.md).
