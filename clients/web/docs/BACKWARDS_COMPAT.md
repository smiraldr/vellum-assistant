...
| `use-supports-image-gen-openrouter-provider.ts` | `0.12.1-dev.0`                         | OpenRouter is hidden in the image-generation provider picker. Older daemons reject `provider: "openrouter"` and have no legacy write shape for it. Pending-feature floor: the enum has not landed on main, so an exact `dev-release.yaml` stamp would light up routeless main builds. Released 0.12.1 is excluded; 0.12.1-dev / 0.12.1-local and later bases pass. Rewrite the floor to the first main-line stamp after merge.                                                                                                                                                                                     | The picker offers OpenRouter and the save path writes `provider: "openrouter"`                                                                                                                                                                                                                                                                                                                                                                                                         |
